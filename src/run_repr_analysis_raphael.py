...
os.chdir("C:/Users/rapha\Documents/ni_git/local-error-signals/src")
...
os.chdir("C:/Users/rapha\Documents/ni_git/local-error-signals/src/theoretical_framework_for_target_propagation")
...
'''models from own backprop skript'''
from AllCNNC_backprop import AllCNNC, AllCNNC_short_kernel
# backprop_short
# model = AllCNNC_short_kernel()
# model.load_state_dict(torch.load('./logs/backprop_ACNNC_short/weights_backprop.pth', 
...
# model = load_network_w_weights(args, "./logs/4_shortACNNC_100_kernel_adapt")


'''models from local error paper'''
...
def remove_layer_backprop(modules):
    del modules[-2]
    del modules[-1] # remove avg_layer, flatten layer
    return modules
...
train_set = datasets.CIFAR10('../theoretical_framework_for_target_propagation/data', train=True, download=True)
train_loader = gen_train_loader(train_set, **kwargs)

test_set = datasets.CIFAR10('../theoretical_framework_for_target_propagation/data', train=False)
